<?php
    ...
    if (!isset($_GET["power"])) $power = 50; else {
        $power = $_GET["power"];
    ...
    // Sprawdź, czy "power" jest większe od 250. Jeśli tak, daj błąd.
    if($power > 250) return require "errors/453.php";

    // Sprawdź, czy "power" jest mniejsze od -250. Jeśli tak, daj błąd.
    if($power < -250) return require "errors/454.php";
    ...
    imagefilter($image, IMG_FILTER_BRIGHTNESS, $power);
